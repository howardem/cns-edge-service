package com.polarbookshop.gateway.config;
...
public class SecurityConfiguration {
	...
	private ServerLogoutSuccessHandler oidcLogoutSuccessHandler(ReactiveClientRegistrationRepository clientRegistrationRepository) {
		OidcClientInitiatedServerLogoutSuccessHandler oidcClientInitiatedServerLogoutSuccessHandler = new OidcClientInitiatedServerLogoutSuccessHandler(clientRegistrationRepository);
		...
		return (exchange, chain) -> {
			exchange.getResponse()
				.beforeCommit(() -> Mono.defer(() -> {
					Mono<CsrfToken> csrfTokenStream = exchange.getAttribute(CsrfToken.class.getName());
					return csrfTokenStream != null ? csrfTokenStream.then() : Mono.empty();
				}));
